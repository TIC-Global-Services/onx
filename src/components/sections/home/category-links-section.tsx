        ...
        <div className="w-full">
          ...
          {categories.map((category) => (
            <Link
              key={category.id}
              href={category.slug}
              className="group flex flex-wrap items-center justify-start gap-4 md:gap-8 py-8 md:py-12 border-b border-onx-border-light hover:border-onx-red/40 transition-colors"
            >
              ...
              <span className="inline-flex items-center gap-4 group/name">
                ...
                <span className="w-[40px] h-[28px] md:w-[60px] md:h-[40px] flex-shrink-0 flex items-center justify-center mix-blend-multiply transition-all duration-300 group-hover:translate-y-2">
                  {category.name.toLowerCase() === 'rifles & guns' && (
